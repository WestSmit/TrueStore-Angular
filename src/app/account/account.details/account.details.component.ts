...
export class AccountDetailsComponent implements OnInit {
  ...
  constructor(private loginService: LoginService, private formBuilder:FormBuilder, private accountService:AccountService) {
  ...
  ngOnInit(): void {
    this.infoForm = this.formBuilder.group(
      {
        firstName: [this.user.firstName, [Validators.required, Validators.pattern("[A-Za-z]{2,}")]],
        secondName: [this.user.secondName, [Validators.required, Validators.pattern("[A-Za-z]{2,}")]],
        userName: [this.user.userName, [Validators.required, Validators.pattern("[A-Za-z0-9]{2,}")]],
        phoneNumber: [this.user.phoneNumber, [Validators.required, Validators.pattern("[0-9]{11}")]],
      }
    );
  ...
  async updateInfo(){  
    this.infoFormResult = await this.accountService.updateUserInfo(this.infoForm.value);
    this.infoformMessage = this.accountService.message;
    this.infoFormCompleted = true;
  }

  async changeEmail(){
    this.emailFormResult = await this.accountService.changeEmail(this.user.id, this.emailForm.controls.email.value);
