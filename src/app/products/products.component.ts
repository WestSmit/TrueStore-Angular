...
export class ProductsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activateRoute.queryParamMap.subscribe((params) => {
      this.productService.getProducts(params).then((data) => {
        ...
        this.filterBrandOptions = this.searchResult.brands.map(
          (x) =>
            new CheckboxItem(
              x.id,
              x.name,
              this.searchResult.selectedBrands?.includes(x.id)
            )
        );
    ...
    this.sortControl.valueChanges.subscribe(() =>
      this.sortOptionsChanged(this.sortControl.value)
    );
  }
  ...
  brandsChanged() {
    let selected = this.filterBrandOptions
